const DraggableUtils = {
      ...
      .draggable({
        listeners: {
          ...
          move: (event) => {
            ...
            // 判断是否骑缝章。骑缝章只允许上下移动
            let groupId = target.getAttribute('data-gapStampGroupId');
            ...
            if (groupId) {
              ...
              for (let i = 0; i < this.pdfDoc.numPages; i++) {
                let draggablesData = pagesMap[i];
                for (let j = 0;  j < draggablesData.length; j++ ) {
                  let ele = draggablesData[j].element;
                  if (ele.getAttribute('data-gapStampGroupId') === groupId) {
                    ele.style.transform = `translate(${x}px, ${y}px)`;
                    ele.setAttribute('data-bs-x', x);
                    ele.setAttribute('data-bs-y', y);
                    // 每页纸只可能有某个骑缝章的一块，没必要继续判断本页纸的元素。
                    break;
                  }
                }
              }
  ...
  createDraggableCanvasFromUrl: function (dataUrl, imgSize) {
    return new Promise((resolve) => {
      ...
      canvasContainer.addEventListener('click', () => {
        ...
        $("#gapStampToolTips").addClass('d-none');
      ...
      myImage.onload = () => {
        ...
        // 计算pdf显示的宽高相对A4纸(宽210mm)的缩放比率，进而设置印章的宽高。
        let rect = document.querySelector("#pdf-canvas").getBoundingClientRect();
        const scaleFactor = Math.floor(rect.width + 0.5) / 210;

        const newWidth = imgSize * scaleFactor;
        const newHeight = imgSize * scaleFactor;
  ...
  createGapStampFromUrl: function (dataUrl, imgSize) {
        return new Promise((resolve) => {
            ...
            for (let i = 0; i < this.pdfDoc.numPages; i++) {
                const canvasContainer = document.createElement('div');
                canvasContainer.id = `draggable-canvas-${this.nextId++}`;
                canvasContainer.setAttribute("data-gapStampGroupId", groupId);
                canvasContainer.classList.add('draggable-canvas');
                ...
                canvasContainer.addEventListener('click', () => {
                    this.lastInteracted = canvasContainer;
                    let x = canvasContainer.getAttribute('data-bs-x') - 20,
                      y = canvasContainer.getAttribute('data-bs-y');
                    $("#gapStampToolTips").removeClass('d-none')
                      .css('transform', `translate(${x}px, ${y}px)`);

                });

                stampParts.push(canvasContainer);
            }

            this.boxDragContainer.appendChild(stampParts[this.pageIndex]);
            ...
            myImage.onload = () => {
                ...
                const newWidth = Math.floor(imgSize * scaleFactor / this.pdfDoc.numPages + 0.5);
                const newHeight = imgSize * scaleFactor;

                // 计算骑缝章在每页的宽度
                const eachWidth = Math.floor(myImage.width / this.pdfDoc.numPages + 0.5);
                for (let i = 0; i < stampParts.length; i++) {
                  const canvasContainer = stampParts[i];
                  const createdCanvas = document.createElement('canvas'); // Inner canvas
                    ...
                    let x = rect.width - newWidth - padding -1, // 边框 1px
                        y = rect.height / 2 - newHeight / 2,
                        angle = 0;
                    canvasContainer.style.transform = `translate(${x}px, ${y}px)`;
                    canvasContainer.setAttribute('data-bs-x', x);
                    canvasContainer.setAttribute('data-bs-y', y);
                    ...
                    canvasContainer.style.padding = `${padding}px `;
                    canvasContainer.style.width = `${newWidth + 2 * padding + 2}px`; // 左右边框占用了2px
                    canvasContainer.style.height = `${newHeight + 2 * padding}px`;

                    context.imageSmoothingEnabled = true;
                    context.imageSmoothingQuality = 'high';
                    context.drawImage(myImage, eachWidth * i, 0, eachWidth, myImage.height, 0, 0, myImage.width, myImage.height);
                    ...
                    pagesMap[i] = draggablesData;
                    pagesMap[i + '-offsetWidth'] = this.pdfCanvas.offsetWidth;
                    pagesMap[i + '-offsetHeight'] = this.pdfCanvas.offsetHeight;
                }

                this.documentsMap.set(this.pdfDoc, pagesMap);

                this.lastInteracted = stampParts[this.pageIndex];

                let x = rect.width - newWidth - padding -1 - 20, // 边框 1px
                y = rect.height / 2 - newHeight / 2;
                $("#gapStampToolTips").removeClass('d-none')
                  .css('transform', `translate(${x}px, ${y}px)`);

                resolve(this.lastInteracted);
            };
  ...
  deleteDraggableCanvas(element) {
    if (element) {
      ...
      // 当所有印章均已删除时
      if ($(element).parent().children(".draggable-canvas").length === 1) {
        // 隐藏删除按钮
        $("#btnDeleteStamp").addClass("d-none");
        // 隐藏旋转按钮
        //this.hideRotationControls();
      }

      const groupId = element.getAttribute('data-gapStampGroupId');
      // 删除骑缝章时，所有部分一并删除
      if (groupId) {
          let pagesMap = this.documentsMap.get(this.pdfDoc);
          for (let i = 0; i < this.pdfDoc.numPages; i++) {
              let draggablesData = pagesMap[i];
              for (let j = 0;  j < draggablesData.length; j++ ) {
                  let item = draggablesData[j];
                  if (item.element.getAttribute('data-gapStampGroupId') === groupId) {
                      draggablesData.pop(j);
                      // 每页纸只可能有某个骑缝章的一块，没必要继续判断本页纸的元素。
                      break;
                  }
              }
    ...
    for (let pageIdx in pagesMap) {
      ...
      for (const draggableData of draggablesData) {
        ...
        const draggablePositionPixels = {
          x: translateX + childOffsetX + this.padding + 1,
          y: translateY + childOffsetY + this.padding + 1,
